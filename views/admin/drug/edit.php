	...
	<div class="title">
		<div class="row">	
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<h1> <a href="#">Sửa thông tin thuốc </a></h1> <hr>
				<ol class="breadcrumb">
                    <li><a href="#">Trang chủ</a></li>
                    <li><a href="dashboard.php?page=drug&action=show">Quản lý thuốc </a></li>
                    <li><a href="dashboard.php?page=drug&action=edit">Sửa thông tin thuốc</a></li>
				</ol>	
		...
		<div class="panel panel-default">
		  	<div class="panel-heading">Thông tin sản phẩm</div>
		  		<div class="panel-body">
                    <form id="edit_drug" method="post" action="dashboard.php?page=drug&action=edit&status=return"
                    enctype="multipart/form-data">
                        
                    <input type="hidden" name="drug_id" value="<?= $drug->drug_id?>"/>
                    <div class="form-group">
			        	<label for="drug_category_id">Nhóm thuốc</label>
			        	<input type="text" name="drug_category_id" class="form-control" value="<?=  $drug->drug_category_id?>" > 
		        	</div>
                    <div class="form-group">
			        	<label for="drug_name">Tên thuốc</label>
			        	<input type="text" name="drug_name" class="form-control" value="<?= $drug->drug_name?>" > 
		        	</div>
                   <div class="form-group">
			        	<label for="description">Miêu tả </label>
			        	<textarea name="description" rows="3" class="form-control" 
			        	value="<?= $drug->description?>" > <?= $drug->description?></textarea>
		        	</div>
                    <div class="form-group">
			        	<label for="ingredient">Thành phần</label>
			        	<textarea name="ingredient"  rows="5" class="form-control" 
			        	value="<?= $drug->ingredient?>" ><?= $drug->ingredient?></textarea>
		        	</div>
                    <div class="form-group">
			        	<label for="guide_to_use">Cách dùng</label>
			        	<textarea name="guide_to_use"  rows="5" class="form-control" 
			        	value="<?= $drug->guide_to_use?>" ><?= $drug->guide_to_use?></textarea>
		        	</div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
			        	...
			        	<input type="text" name="price" class="form-control" value="<?= $drug->price?>" > 
		        	</div>
                    <div class="form-group">
